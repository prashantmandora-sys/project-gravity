...
A static debt-tracking dashboard + payoff simulator, seeded from `My Finances.xlsx` (Debts sheet).

- **Frontend**: plain HTML/CSS/JS, no build step, no framework. Charts via Chart.js (CDN).
- **Data**: seeded once from `data-seed.js` into `localStorage`; edit/add/delete debts in the app from then on. Use the Export/Import buttons to back up or move data between browsers/devices.
- **Simulator**: snowball vs avalanche payoff simulation, runs entirely client-side.
- **AI features** (`/api/ai-commentary.js`, `/api/parse-statement.js`): Vercel serverless functions that call the **Google Gemini API** (free tier, no credit card needed). Requires a `GEMINI_API_KEY` environment variable set in the Vercel project — never commit the key.
